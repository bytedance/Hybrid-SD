...
calc_is() {
    ...
    fidelity --gpu $GPU_NUM --isc --input1 $1/$2 | tee $IS_TXT
    echo "============"
}
...
calc_fid(){
    ...
    CUDA_VISIBLE_DEVICES=$GPU_NUM python3 -m pytorch_fid --save-stats $1/$2 $NPZ_NAME_gen
...
calc_clip() {
    ...
    DATA_LIST=$DATA_ROOT/mscoco_val2014_30k/metadata.csv
    CUDA_VISIBLE_DEVICES=$GPU_NUM python3 evaluation/clip_score.py --img_dir $1/$2 --data_list $DATA_LIST --save_txt $CLIP_TXT
    echo "============"
}
...
do
    ...
    #calc_is $OUTPUT_DIR $VAE
    calc_fid $OUTPUT_DIR $VAE
    #calc_clip $OUTPUT_DIR $VAE
done
