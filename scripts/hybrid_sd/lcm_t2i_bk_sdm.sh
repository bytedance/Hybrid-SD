...
#STUDENT_DIR="results/finetune/NaivePrune/a19_b21"
...
OUTPUT_DIR="results/lcm_sd_teacher_ours_224_tea_sd14" # please adjust it if needed
...
CUDA_VISIBLE_DEVICES=$GPU_NUM accelerate launch \
  --main_process_port 23023 \
  --num_processes=2 \
  --multi_gpu \
  --num_machines=1 \
  examples/prune_sd/train_lcm_distill_bk_sdm_wds.py \
