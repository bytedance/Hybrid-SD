...
def conv_2d(inp, oup, kernel_size=3, stride=1, groups=1, bias=False, norm=False, act=True):
    conv = nn.Sequential()
...
class MultiQueryAttentionLayerWithDownSampling(nn.Module):
    def __init__(self, inp, num_heads, key_dim, value_dim, query_h_strides, query_w_strides, kv_strides, dw_kernel_size=3, dropout=0.0):
        ...
        # self.avg_pool = torch.nn.AvgPool2d(self.query_h_strides, self.query_w_strides)
...
def build_blocks(layer_spec):
    ...
    if block_names == "convbn":
        ...
        for i in range(layer_spec['num_blocks']):
            ...
            if mhsa:
                mhsa = [2, 16, 16, 1, 1, 2, False, True, True]
                mhsa_schema_ = [
                    "inp", "num_heads", "key_dim", "value_dim", "query_h_strides", "query_w_strides", "kv_strides", 
...
class EncoderUIB(nn.Module):
    r"""
    The `EncoderUIB` layer is consis of Universal Inverted Bottleneck (UIB) proposed by MobileNetV4.
    ...
    def __init__(
        ...
        layers = []
        for i, (name, spec) in enumerate(self.model_desc.items()):
            spec = self.model_desc[name]
            layers.append(build_blocks(spec))
...
class DecoderUIB(nn.Module):
    r"""
    The `DecoderUIB` layer is consis of Universal Inverted Bottleneck (UIB) proposed by MobileNetV4.
    ...
    def __init__(
        ...
        for i, (name, spec) in enumerate(self.model_desc.items()):
            spec = self.model_desc[name]
            layers.append(build_blocks(spec))
            is_final_block = i == (len(self.model_desc) - 1)

            if not is_final_block and not i == 0:
...
class AutoencoderUIB(AutoencoderTiny):
    ...
    def __init__(
        ...
        encoder_model_desc: dict = None,
        decoder_model_desc: dict = None,
    ):
        super().__init__()

        if encoder_model_desc is not None:
            self.encoder = EncoderUIB(
                model_desc=encoder_model_desc
            )

        self.decoder = DecoderUIB(
            model_desc=decoder_model_desc,
