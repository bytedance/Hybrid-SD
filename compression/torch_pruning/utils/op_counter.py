...
def count_ops_and_params(model, example_inputs, layer_wise=False):
    ...
    model_copy = copy.deepcopy(model) # deepcopy to avoid changing the original model
    flops_model = add_flops_counting_methods(model_copy)
    flops_model.eval()
    ...
    layer_flops = {}
    layer_params = {}
    for ori_m, m in zip(model.modules(), model_copy.modules()):
