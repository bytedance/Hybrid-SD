...
class MetaPruner:
    ...
    def __init__(
        ...
        max_pruning_ratio_dict: typing.Dict[nn.Module, float] = None, # maximum pruning ratio. useful if over-pruning happens.
        ...
        min_channels: typing.List[nn.Module] = None,
        ...
        self.max_pruning_ratio_dict = max_pruning_ratio_dict if max_pruning_ratio_dict is not None else {}
        ...
        # self.prunin
        # if max_pruning_ratio_dict is not None:
        #     for module in max_pruning_ratio_dict:
        #         ratio = pruning_ratio_dict[module]
        #         for submodule in module.modules():
    ...
    def prune_local(self) -> typing.Generator:
        ...
        for group in self.DG.get_all_groups(ignored_layers=self.ignored_layers, root_module_types=self.root_module_types):
            if self._check_pruning_ratio(group): # check pruning ratio
                ...
                # print(f'estimate_importance for group {group._group[0].dep.source.name}')
                ...
                if self.min_channels is not None and group._group[0].dep.source.name in self.min_channels:
                    n_keep_channels = current_channels - n_pruned
                    if n_keep_channels < self.min_channels[group._group[0].dep.source.name]:
                        logger.info(f"Meet min channels {n_keep_channels} for group {group._group[0].dep.source.name}")
                        n_keep_channels = self.min_channels[group._group[0].dep.source.name]
                        group.valid = False
                        yield group
