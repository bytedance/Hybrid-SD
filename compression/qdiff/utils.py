...
from .quant_layer import QuantModule, UniformAffineQuantizer, TimewiseUniformQuantizer, ActUniformQuantizer
from .quant_block import BaseQuantBlock, QuantBasicTransformerBlock, QuantQKMatMul, QuantSMVMatMul
from .quant_model import QuantModel
from .adaptive_rounding import AdaRoundQuantizer
from .linear_quant import LinearQuantizer
import copy
...
def convert_channelwise_linear(model):
    for name, module in model.named_children():
        if isinstance(module, QuantModule):
            if module.ignore_reconstruction is True:
                ...
                if module.split != 0:
                    module.weight_quantizer = LinearQuantizer(uaq=module.weight_quantizer,
                                                        weight_tensor=module.org_weight.data[:, :module.split, ...])
                    module.weight_quantizer_0 = LinearQuantizer(uaq=module.weight_quantizer_0,
                                                        weight_tensor=module.org_weight.data[:, module.split:, ...])
                else:
                    module.weight_quantizer = LinearQuantizer(uaq=module.weight_quantizer,
                ...
                for name, sub_module in module.named_modules():
                    if isinstance(sub_module, QuantModule):
                        if sub_module.split != 0:
                            logger.info(f"[convert_adaround] split {name}")
                            sub_module.weight_quantizer = LinearQuantizer(uaq=sub_module.weight_quantizer,
                                                                    weight_tensor=sub_module.org_weight.data[:, :sub_module.split, ...])
                            sub_module.weight_quantizer_0 = LinearQuantizer(uaq=sub_module.weight_quantizer_0,
                                                                    weight_tensor=sub_module.org_weight.data[:, sub_module.split:, ...])
                        else:
                            sub_module.weight_quantizer = LinearQuantizer(uaq=sub_module.weight_quantizer,
                                                                    weight_tensor=sub_module.org_weight.data)
        else:
            convert_channelwise_linear(module)
...
def resume_cali_model(qnn, ckpt_path, cali_data, quant_act=False, cond=False):
    # logger.info(f"Resume quantized model checkpoint from {ckpt_path}")
    ckpt = torch.load(ckpt_path, map_location='cpu')
    qnn.set_quant_state(True, False)
    
    # forward once to get split dimension
    if cond:
...
def resume_cali_model_v2(qnn, ckpt_path, cali_data, quant_act=False, cond=False):
    qnn.set_quant_state(True, False)
    ...
    # change weight quantizer from uniform to adaround
    convert_channelwise_linear(qnn)
