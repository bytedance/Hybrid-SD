        ...
        def forward(
            ...
            **cross_attention_kwargs,
            ...
            if gate_step == self.cur_step:
                self.cache = cache
            #print(self.cur_step, hidden_states.shape)
...
# plot differences between adjent cross attention maps 
def plot_register_tgate_forward(
    model, 
    filter_name: str = 'Attention', 
    gate_step: int = 10,
    inference_num_per_image: int = 25,
        ...
        **kwargs,
        ...
        def forward(
            ...
            **cross_attention_kwargs,
            ...
            if not hasattr(self,'cache'):
                self.cache = None
            if not hasattr(self,'pre_cache'):
                """cache cross attention maps from previous timesteps
                """
                self.pre_cache = None
            ...
            if self.pre_cache is not None and self.cur_step > 1:
                self.diffs[str(self.cur_step)] = torch.mean(torch.abs(hidden_states - self.pre_cache)) #torch.mean(hidden_states - self.pre_cache) #
                
            self.pre_cache  = hidden_states
            if gate_step == self.cur_step:
                self.cache = cache
            if self.cur_step == inference_num_per_image:
                # end of the current inference, clear cache
                del self.cache
                del self.pre_cache
