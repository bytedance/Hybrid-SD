...
step_list=["25,0", "5,20", "2,23", "0,25"]
# prompt_id=0
# image_id=0
# for prompt_id in range(7):
#     for image_id in range(6):
        # if prompt_id == 0 and image_id == 0:
            # continue
prompt_id=0
image_id=4
...
for step in step_list:
    ...
    middle_imgs = []
    plot_steps = list(range(0, 25, 5))
    if 24 not in plot_steps:
        plot_steps.append(24)

    for i in plot_steps:
        if i < k:
            img_name = f'{MODEL_LARGE}_step'
        else:
            img_name = f'{MODEL_SMALL}_step'
        img_path = middle_imgs_path / f'{img_name}_{i}.png'
        img_paths.append(img_path)
    print(len(img_paths))
model_names=['Realistic_Vision_V5.1', 'Realistic-V5.1 + segmind-small (k=5)', 'Segmind-Small']
grid_plot(img_paths, step_list, plot_steps, n_row=len(step_list), save_path=WORKSPACE / f'notebooks/figures/realistic-{prompt_id}_image-{image_id}.png')
# except:
#     continue
