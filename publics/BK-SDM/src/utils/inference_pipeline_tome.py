...
class InferencePipeline:
    def __init__(self, weight_folder, seed, device, tome_args, deepcache_args, tgate_args):
        self.weight_folder = weight_folder
        self.seed = seed
        self.device = torch.device(device)
        self.tome_args = tome_args
        self.deepcache_args = deepcache_args
        self.tgate_args = tgate_args
    ...
    def set_pipe_and_generator(self): 
        ...
        if self.tome_args['tome']:
            tomesd.apply_patch(self.pipe, ratio=0.5)
        
        if self.deepcache_args['deepcache']:
            self.pipe = DeepCacheStableDiffusionPipeline.from_pretrained(self.weight_folder, torch_dtype=torch.float16).to(self.device)

        if self.tgate_args['tgate']:
            self.pipe = TgateSDLoader(
                self.pipe,
                gate_step=self.tgate_args['tgate_step'],
    ...
    def generate(self, prompt: Union[str, List[str]], n_steps: int, img_sz: int) -> List[Image.Image]:
        if self.tgate_args['tgate']:
            out = self.pipe.tgate(
                prompt,
                num_inferece_steps = n_steps,
                height = img_sz,
                width = img_sz,
                generator = self.generator,
                gate_step = self.tgate_args['tgate_step']
            )
        elif isinstance(self.pipe, DeepCacheStableDiffusionPipeline):
            out = self.pipe(
                prompt, 
                cache_interval=self.deepcache_args['cache_internal'], cache_layer_id=0, cache_block_id=0,
