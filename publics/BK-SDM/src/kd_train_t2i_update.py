...
def main():
    ...
    # Init slim unet
    assert args.unet_config_name in ["bk_small", "bk_tiny"]
    unet = UNet2DConditionModel.from_pretrained(
        args.pretrained_model_name_or_path, subfolder="unet", revision=args.non_ema_revision
    )
    if args.prepare_unet=="True":
        prepare_unet(unet, args.unet_config_name, accelerator)
    ...
    KD_teacher = {}
    KD_student= {}    
    add_hook_update(unet, KD_student, args.unet_config_name, accelerator, False)
    add_hook_update(unet_teacher, KD_teacher, args.unet_config_name, accelerator, True)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet):
                ...
                loss_kd_feat = 0.0
                if args.unet_config_name == "bk_small":
                    for i in range(4):
                        loss_kd_feat=loss_kd_feat+F.mse_loss(KD_teacher['d'+str(i)],KD_student['d'+str(i)])
                    loss_kd_feat=loss_kd_feat+F.mse_loss(KD_teacher['m'],KD_student['m'])
                    for i in range(4):
                        loss_kd_feat=loss_kd_feat+F.mse_loss(KD_teacher['u'+str(i)],KD_student['u'+str(i)])
                else:
                    for i in range(2):
                        loss_kd_feat=loss_kd_feat+F.mse_loss(KD_teacher['d'+str(i)],KD_student['d'+str(i)])
                    loss_kd_feat=loss_kd_feat+F.mse_loss(KD_teacher['u'+str(0)],KD_student['d'+str(2)])
                    for i in range(3):
                        loss_kd_feat=loss_kd_feat+F.mse_loss(KD_teacher['u'+str(i+1)],KD_student['u'+str(i)])
