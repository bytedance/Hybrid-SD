...
# from compression.prune_sd.prune_utils import prune_unet, recover_unet, calculate_score, set_choice
...
def create_pipeline(vae, text_encoder, tokenizer, unet, args, accelerator, weight_dtype, teacher=False):
    if not teacher:
...
def log_validation(vae, text_encoder, tokenizer, unet, args, accelerator, weight_dtype, epoch_or_step, logger, teacher=False, val_img_dir=None):
    logger.info("Running validation... ")
    if not teacher:
        pipeline, generator = create_pipeline(vae, text_encoder, tokenizer, unet, args, accelerator, weight_dtype, teacher)
        images = []
        for i in range(len(args.valid_prompt)):
            with torch.autocast("cuda"):
                image = pipeline(args.valid_prompt[i], num_inference_steps=25, generator=generator).images[0]
            images.append(image)
        for tracker in accelerator.trackers:
            if tracker.name == "tensorboard":
                np_images = np.stack([np.asarray(img) for img in images])
                tracker.writer.add_images("validation", np_images, epoch_or_step, dataformats="NHWC")
            elif tracker.name == "wandb":
                tracker.log(
                    {
                        "validation": [
                            wandb.Image(image, caption=f"{i}: {args.valid_prompt[i]}")
        ...
        if val_img_dir:
            for index, image in enumerate(images):
                image.save(os.path.join(val_img_dir, f"gstep{epoch_or_step}_{index}.png"))
    else:
        # create pipeline
        pipeline, generator = create_pipeline(vae, text_encoder, tokenizer, unet, args, accelerator, weight_dtype, teacher)
        th_images = []
        ...
        if  val_img_dir:
            if not os.path.exists(os.path.join(val_img_dir, "teacher_0.png")):
                for index, image in enumerate(th_images):
                    image.save(os.path.join(val_img_dir, f"teacher_{index}.png"))
            for index, image in enumerate(images):
                image.save(os.path.join(val_img_dir, f"gstep{epoch_or_step}_{index}.png"))
    del pipeline
...
def parse_args():
    ...
    parser.add_argument("--valid_prompt", 
                type=str, 
                default="a golden vase with different flowers",
                nargs="+",
                help=("A set of prompts evaluated every `--valid_steps` and logged to `--report_to`."),)
    ...
    parser.add_argument("--n_prune_choices", type=int, default=5)
    parser.add_argument("--prune_steps", type=int, default=500)
    parser.add_argument("--stop_prune_steps", type=int, default=20000)
    parser.add_argument("--prune_iterative_steps", type=int, default=2)
    ...
    if args.prune_steps % args.gradient_accumulation_steps != 0:
        raise ValueError("prune_steps should be a multiple of gradient_accumulation_steps")
...
def main():
    ...
    args.output_dir = logging_dir = os.path.join(args.output_dir, cur_time)
    ...
    # Init slim unet
    # config_student = UNet2DConditionModel.load_config(args.pretrained_model_name_or_path, subfolder="unet")
    ...
    pipeline, generator = create_pipeline(vae, text_encoder, tokenizer, unet, args, accelerator, weight_dtype=torch.float32, teacher=True)
    teacher_latent = None
    ...
    with torch.autocast("cuda"):
        teacher_latent = pipeline(prompt = val_prompts,
                        n_steps = 25,
                        generator = generator,
                        output_type = "latent").images
        teacher_latent = teacher_latent.cpu().detach().numpy()

    example_inputs = [torch.randn((2, 4, 64, 64), dtype=torch.float32).to(unet.device), torch.randn(1, dtype=torch.float32).to(unet.device), torch.randn((2, 77, 768), dtype=torch.float32).to(unet.device)]
    ...
    final_choices = []
    unet.eval()
    for _ in range(args.prune_iterative_steps):
        latents = [teacher_latent]
        choices = []

        for choice in pruner.candidate_choices:
            print(f'prune choice={choice}')
            pruner.set_choice(choice)
            success = pruner.prune()
            if not success:
                pruner.candidate_choices.remove(choice)
                continue

            choices.append(choice)
            output_latent = pipeline(prompt = val_prompts,
                n_steps = 25,
                generator = generator,
                output_type = "latent").images
            np_latents = output_latent.cpu().detach().numpy()
            latents.append(np_latents)
            pruner.recover()
            break
            
        scores = calculate_score(latents)
        logger.info(f'choices={choices}, scores={scores}')
        top1_idx = np.argsort(scores)[0]
        top1_choice = choices[top1_idx]
        final_choices.append(top1_choice)
        pruner.set_choice(choice)
        pruner.prune()
        ...
        ratio_flops = (org_flops - flops) / org_flops * 100
        logger.info(f'top1_choice={top1_choice} params: {params} ({ratio_params:.2f}%), macs: {macs} ({ratio_macs:.2f}%), flops: {flops} ({ratio_flops:.2f}%)\n')

    ratio_macs = (org_macs - macs) / org_macs * 100
    ratio_params = (org_params - params) / org_params * 100
    ratio_flops = (org_flops - flops) / org_flops * 100
    logger.info(f'Final choice={final_choices} params: {params} ({ratio_params:.2f}%), macs: {macs} ({ratio_macs:.2f}%), flops: {flops} ({ratio_flops:.2f}%)\n')
    unet = pruner.unet

    logger.info("validate after pruning...")
    log_validation(
        vae,
        text_encoder,
        tokenizer,
    ...
    # Open gradient_checkpointing for saving mem.
    ...
    logger.info(f"*** load dataset {args.dataset_name} {args.train_data_dir}: start")
    t0 = time.time()
    if "laion-art" in args.dataset_name or "laion_art" in args.dataset_name:
    ...
    unet, optimizer, train_dataloader, lr_scheduler, unet_teacher = accelerator.prepare(
        unet, optimizer, train_dataloader, lr_scheduler, unet_teacher
    )
    if args.use_ema:
        ema_unet.to(accelerator.device)
    ...
    if accelerator.is_main_process:
        ...
        tracker_config.pop("valid_prompt")
    ...
    if torch.cuda.device_count() > 1:
        logger.info(f"use multi-gpu: # gpus {torch.cuda.device_count()}")
