...
DATASET_NAME_MAPPING = {
    ...
    "/mnt/bn/bytenn-data2/laion-art": ("image", "text"),
}
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=int,
        default=256,  # The value used in the SimianLuo/LCM_Dreamshaper_v7 ckpt, but you may want to change this
        help=(
            "The dimension of the guidance scale embedding to the U-Net, which will be used if the teacher U-Net"
...
def main(args):
    ...
    if "laion-art" in args.dataset_name or "laion_art" in args.dataset_name:
    ...
    elif "laion-aes" in args.dataset_name:
        dataset = load_dataset(
            "imagefolder", 
        ...
        raise NotImplementedError("Don`t support this datasets!")
        from utils.dataset import Text2ImageDataset
    ...
    def tokenize_captions(examples, proportion_empty_prompts=0, is_train=True):
        captions = []
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                image = image.to(device=accelerator.device, dtype=weight_dtype, non_blocking=True)
                prompt_embeds = text_encoder(text_ids.to(text_encoder.device))[0]
