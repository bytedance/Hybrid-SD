...
from ADD.utils.scheduling_ddpm import DDPMScheduler
...
def parse_args():
    ...
    parser.add_argument("--lambda_dis", type=float, default=1.0, help="weighting for the denoising task loss")  
    parser.add_argument("--lambda_adv_G", type=float, default=1.0, help="weighting for the generator in adversarial in adversarial training")  
    parser.add_argument("--r1_lamda", type=float, default=0.0, help="weighting for the penalty")  
    parser.add_argument("--project_cdim", type=int, default=0, help="cdim for ProjectDiscriminator")  
...
def main():
    ...
    model_dis = ProjectedDiscriminator(c_dim=args.project_cdim).train()
        ...
        def save_model_hook(models, weights, output_dir):
            ...
            for i, model in enumerate(models):
                if isinstance(model, UNet2DConditionModel):
                    ...
                    weights.pop()
                elif isinstance(model, ProjectedDiscriminator):
                    state_dict = model.state_dict()
                    model_name = type(model).__name__
                    save_directory = os.path.join(output_dir, f'{model_name}.pt')
    ...
    unet, optimizer, train_dataloader, lr_scheduler, model_dis, optimizer_D = accelerator.prepare(
        unet, optimizer, train_dataloader, lr_scheduler, model_dis, optimizer_D
    )
    ...
    unet_teacher.to(accelerator.device, dtype=torch.float32)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        train_loss = 0.0
        train_loss_adv_G = 0.0
        train_loss_adv_D = 0.0
        train_loss_dis = 0.0
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet):
                ...
                if p > 0.5:
                    with torch.no_grad():
                        model_pred_stu = unet(noisy_latents, timesteps_stu, encoder_hidden_states).sample
                else:
                    model_pred_stu = unet(noisy_latents, timesteps_stu, encoder_hidden_states).sample
                ...
                with torch.no_grad():
                    ...
                    counter = 0
                    # teacher prediction
                    for z_t in z_t_tea_list:
                        cur_t_tea = t_tea[counter]
                        model_pred_teacher = unet_teacher(z_t, cur_t_tea, encoder_hidden_states.to(torch.float32)).sample
                        z0_tea = noise_scheduler.get_z0(z_t, model_pred_teacher, cur_t_tea)
                ...
                # model_dis.requires_grad_(True)
                # model_dis.dino.requires_grad_(False)

                pred_real, features = model_dis(pixel_values, c=encoder_hidden_states)
                pred_fake, _ = model_dis(x0_stu.detach(), c=encoder_hidden_states)
                ...
                if args.r1_lamda != 0:
                    ...
                    for i in range(0, len(pred_real)):
                        ...
                        grad_penalty = (grad_real.contiguous().view(grad_real.size(0), -1).norm(2, dim=1) ** 2).mean() * args.r1_lamda
                ...
                loss_adv_D = loss_real + loss_fake
                accelerator.backward(loss_adv_D)
                optimizer_D.step()
                optimizer_D.zero_grad()
                # model_dis.requires_grad_(False)
                ######### Phase D End #########

                ######### Phase G Start #########
                # unet.requires_grad_(True)
                # compute the generator loss
                pred_fake, _ = model_dis(x0_stu, c=encoder_hidden_states)
                pred_fake = torch.cat(pred_fake, dim=1)
                loss_adv_G = -torch.mean(pred_fake)
                ...
                loss_dis = (dis_loss1.mean() + dis_loss2.mean()) / 2
                loss = args.lambda_dis * loss_dis + args.lambda_adv_G * loss_adv_G

                # Gather the losses across all processes for logging (if we use distributed training).
                avg_loss = accelerator.gather(loss.repeat(args.train_batch_size)).mean()
                train_loss += avg_loss.item() / args.gradient_accumulation_steps

                avg_loss_adv_G = accelerator.gather(loss_adv_G.repeat(args.train_batch_size)).mean()
                train_loss_adv_G += avg_loss_adv_G.item() / args.gradient_accumulation_steps

                avg_loss_dis = accelerator.gather(loss_dis.repeat(args.train_batch_size)).mean()
                train_loss_dis += avg_loss_dis.item() / args.gradient_accumulation_steps

                avg_loss_adv_D = accelerator.gather(loss_adv_D.repeat(args.train_batch_size)).mean()
                train_loss_adv_D += avg_loss_adv_D.item() / args.gradient_accumulation_steps
            ...
            if accelerator.sync_gradients:
                ...
                accelerator.log(
                    {
                        ...
                        "train_loss_dis": train_loss_dis,
                        "train_loss_adv_G": train_loss_adv_G,
                        "train_loss_adv_D": train_loss_adv_D,
                ...
                if accelerator.is_main_process:
                    with open(csv_log_path, 'a') as logfile:
                        logwriter = csv.writer(logfile, delimiter=',')
                        logwriter.writerow([epoch, step, global_step,
                                            train_loss, train_loss_adv_G, train_loss_adv_D, train_loss_dis,
                                            lr_scheduler.get_last_lr()[0],
                                            args.lambda_dis, args.lambda_adv_G])
            ...
            logs = {"step_loss": loss.detach().item(),
                    "adv_G_loss": loss_adv_G.detach().item(),
                    "adv_D_loss": loss_adv_D.detach().item(),
                    "dis_loss": loss_dis.detach().item(),
                    "lr": lr_scheduler.get_last_lr()[0]}
