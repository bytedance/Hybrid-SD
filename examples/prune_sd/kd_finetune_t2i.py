...
from compression.utils.misc import (
    add_hook,
...
def parse_args():
    ...
    parser.add_argument("--unet_config_name", type=str, default="unet")   
...
def main():
    ...
    example_inputs = [torch.randn((2, 4, 64, 64), dtype=torch.float32).cuda(), torch.randn(1, dtype=torch.float32).cuda(), torch.randn((2, 77, 768), dtype=torch.float32).cuda()]
    ...
    unet_copy = deepcopy(unet_teacher)
    flops, macs, params = calculate_flops(
        ...
        print_results=False)
    logger.info(f"Teacher Model = {args.pretrained_model_name_or_path}, #Params: {params}, #MACS: {macs}")

    # Init slim unet
    logger.info(f"Init Slim Unet from {args.unet_config_path}")
    unet = CustomUNet2DConditionModel.from_pretrained(args.unet_config_path, subfolder="unet", revision=args.non_ema_revision)
    
    unet_copy = deepcopy(unet)
    unet_copy = unet_copy.cuda()
    flops, macs, params = calculate_flops(
            model=unet_copy, 
            args=example_inputs,
            output_as_string=True,
            ...
            print_detailed=False,
            print_results=False)
    logger.info(f"Student Model = {args.unet_config_path}, #Params: {params}, #MACS: {macs}")
    del unet_copy, example_inputs
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet):
                ...
                losses_kd_feat = []
                for mapping_id, (m_tea, m_stu) in enumerate(zip(mapping_layers_tea, mapping_layers_stu)):
