...
if __name__ == "__main__":
    ...
    for g_idx, g in enumerate(pruner.step(interactive=True)):
        logger.info(f'Pruning block [{g_idx}] {g._group[0].dep}')
        g.prune()
        macs, params = tp.utils.count_ops_and_params(unet, example_inputs)
        logger.info(f"#Params: {base_params/1e6:.4f} M => {params/1e6:.4f} M, #MACS: {base_macs/1e9:.4f} G => {macs/1e9:.4f} G")
    ...
    for name, module in unet.named_modules():
        ...
        if isinstance(module, BasicTransformerBlock):
            ...
            unet._internal_dict['crossattn_out_dim'][n_attn_block] = module.attn1.to_v.out_features
            n_attn_block += 1

    unet.save_pretrained(os.path.join(args.save_dir, "unet"))
