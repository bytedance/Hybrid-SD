...
"""
Use a SD1.4 teacher to train lcm version of bk_sdm model
...
def main(args):
    ...
    if args.report_to == 'wandb':
        ...
        config=args,
        init_kwargs={"wandb":{"name":"Finetuned_sd_14_oursTiny224_sd14_LCM"}},
        )
    ...
    unet_weight = CustomUNet2DConditionModel.from_pretrained(
        args.tiny_student_model, subfolder="unet" #, revision=args.non_ema_revision
    )

    unet = CustomUNet2DConditionModel.from_config(unet_weight.config, time_cond_proj_dim=time_cond_proj_dim)
    ...
    target_unet = CustomUNet2DConditionModel.from_config(unet.config, time_cond_proj_dim=time_cond_proj_dim)
