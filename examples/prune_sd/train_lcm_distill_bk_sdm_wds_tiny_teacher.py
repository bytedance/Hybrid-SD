...
class SDText2ImageDataset:
    def __init__(
        ...
        # modified urls 
        train_shards_path_or_url = [os.path.join(train_shards_path_or_url, file_name) for file_name in os.listdir(train_shards_path_or_url) if file_name.endswith('.tar')]
...
def main(args):
    ...
    # teacher_unet = UNet2DConditionModel.from_pretrained(
    #     args.tiny_student_model, subfolder="unet" #, revision=args.non_ema_revision
    # )
    from diffusers import UNet2DConditionModel as SD14_unet
    teacher_unet = SD14_unet.from_pretrained(
        args.pretrained_teacher_model, subfolder="unet" #, revision=args.non_ema_revision
    )
    ...
    ## Load tiny unet models
    unet_weight = UNet2DConditionModel.from_pretrained(
        args.tiny_student_model, subfolder="unet" #, revision=args.non_ema_revision
    )

    unet = UNet2DConditionModel.from_config(unet_weight.config, time_cond_proj_dim=time_cond_proj_dim)
    unet.load_state_dict(unet_weight.state_dict(), strict=False)
    ...
    #add time_cond_proj_dim, since time_cond_proj_dim is newly-added, laod_state_dict should be strict=False
    target_unet = UNet2DConditionModel.from_config(unet.config, time_cond_proj_dim=time_cond_proj_dim)
