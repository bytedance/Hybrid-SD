...
def parse_args():
    ...
    parser.add_argument("--keep_selfattn_heads_ratio", type=int, default=0.5)
    parser.add_argument("--prune_crossattn_layers", type=str, default=None)
    parser.add_argument("--keep_crossattn_heads_ratio", type=int, default=0.5)
...
if __name__ == "__main__":
    ...
    pruner = UnetPruner(args, unet, keep_resnet_ratio=args.keep_resnet_ratio, keep_crossattn_heads_ratio=args.keep_crossattn_heads_ratio, keep_selfattn_heads_ratio=args.keep_selfattn_heads_ratio, example_inputs=example_inputs)
    ...
    save_path = os.path.join(args.save_dir, f'latents.npy')
    np.save(save_path, np_latents)
    print(f'Save latents to {save_path}')
    
    pipeline.clear()
    print(f"{(time.perf_counter()-t0):.2f} sec elapsed")

    flops, macs, params = pruner.calc_flops(output_as_string=True)
